...
const props = defineProps({
    ...
    specialism: {
        type: Object,
        default: () => ({
            selection: { types: [], provinces: [] },
            options: { types: [], provinces: [] },
        }),
    },
    organizations: {
        type: Array,
...
const specialism = computed(() => ({
    selection: props.specialism?.selection ?? { types: [], provinces: [] },
    options: props.specialism?.options ?? { types: [], provinces: [] },
}));
...
const form = useForm({
    ...
    invite: false,
    types:
        specialism.value.selection.types?.length
            ? specialism.value.selection.types
            : specialism.value.options.types ?? [],
    provinces:
        specialism.value.selection.provinces?.length
            ? specialism.value.selection.provinces
            : specialism.value.options.provinces ?? [],
});
...
const organizationId = computed(() => props.organization?.id ?? null);
const backLink = computed(() =>
    organizationId.value
        ? route("admin.organizations.edit", organizationId.value)
        : route("admin.users.index")
);
const backLabel = computed(() =>
    organizationId.value ? "Terug naar Makelaar" : "Terug naar gebruikers"
);

const organizationOptions = computed(() => {
    const organizations = Array.isArray(props.organizations)
        ? props.organizations
        : [];
...
const filteredOrganizations = computed(() => {
    ...
    if (!query) return organizationOptions.value;
    ...
    <AuthenticatedLayout>
        ...
        <div class="py-8">
            <PageContainer class="space-y-6">
                <form @submit.prevent="showInviteModal = true" class="space-y-6">
                    <FormSection>
                    ...
                    <div class="mt-6 space-y-6">
                        ...
                        <div class="relative">
                            ...
                            <TextInput
                                ...
                                @focus="openOrganizationDropdown"
                                @input="openOrganizationDropdown"
                                @blur="closeOrganizationDropdown"
                                @keydown.esc.prevent="showOrganizationDropdown = false"
                            />
                        ...
                        <FormActions align="right">
                            <SecondaryButton
                                ...
                                @click="() => window.location.href = backLink"
                            >
                                Annuleren
                            </SecondaryButton>
